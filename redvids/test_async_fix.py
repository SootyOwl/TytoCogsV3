...
def test_async_download_with_mock():
    ...
    assert "await asyncio.to_thread(downloader.check)" in source_code, (
        "check() not properly awaited"
    )
    assert "await asyncio.to_thread(downloader.download)" in source_code, (
        "download() not properly awaited"
    )
